  ...
  const fetchInvoices = async () => {
    try {
      ...
      if (response.data && response.data.invoices) {
        ...
        const formattedInvoices = response.data.invoices.data || [];
        setInvoices(formattedInvoices);
        setUnpaidTotal(parseFloat(response.data.unpaid_total || 0));
        
        // Calculer les statistiques
        const stats = {
          total: formattedInvoices.length,
          paid: formattedInvoices.filter(inv => inv.status === 'paid').length,
          unpaid: formattedInvoices.filter(inv => inv.status === 'unpaid').length,
          overdue: formattedInvoices.filter(inv => 
